var Game = (function () {
	var Game = (function () {
		function Game(width, height, radius) {
			...
			var up = -(widthLong + widthLong - 1),
				topLeft = -widthLong,
				topRight = -(widthLong - 1);
	...
	var Hex = (function () {
		...
		var Side = (function () {
			...
			Side.prototype = {
				...
				render: function () {
					var sideShift = Math.loop(this.Hex.shift + this.shift, 0, 6);
					var otherHex = this.Hex.sides.links[sideShift];

					if (otherHex) {
						console.log(this.Hex.HTML, otherHex.HTML);

						var opacity = 0;

						if (otherHex.getSide(Math.loop(sideShift + 3, 0, 6)).mark === this.mark) {
							opacity = 1;
							//console.log(this.Hex.HTML, otherHex.HTML);
						}
