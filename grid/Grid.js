				...
				if (view.dirty || size.dirty) {
					...
					context.save();
					{
						...
						while (i < l) {
							...
							if (unit === 0) {
								var renderLeft = (c.minX != null) ? Math.max(c.minX, left) : left;
								var renderRight = (c.maxX != null) ? Math.max(c.maxX, right) : right;
								var renderTop = (c.minY != null) ? Math.max(c.minY, top) : top;
								var renderBottom = (c.maxY != null) ? Math.max(c.maxY, bottom) : bottom;
								if (
									(axis.y > -(c.width / 2)) &&
